...
@router.post("/demo/seed-basic", operation_id="demo_seed_basic", summary="Seed a basic helpdesk demo")
async def demo_seed_basic(frappe: FrappeApiClient = Depends(get_frappe_client)):
    customers = [
        {"customer_name": "Acme Cloud Services", "domain": "acme.example"},
        {"customer_name": "Northwind Logistics", "domain": "northwind.example"},
        {"customer_name": "Contoso Retail", "domain": "contoso.example"},
    ]
    seeded_customers = []
    for customer in customers:
        existing = await frappe.find_one("HD Customer", filters={"customer_name": customer["customer_name"]})
        seeded_customers.append(existing or await frappe.create_doc("HD Customer", customer))

    tickets = [
        {
            "subject": "Cannot access billing dashboard",
            "description": "The customer receives a 403 when opening the billing dashboard.",
            "raised_by": "ops@acme.example",
            "customer": "Acme Cloud Services",
            "priority": "High",
        },
        {
            "subject": "Shipment notification emails are delayed",
            "description": "Email notifications are arriving 30 minutes after shipment updates.",
            "raised_by": "support@northwind.example",
            "customer": "Northwind Logistics",
            "priority": "Medium",
        },
        {
            "subject": "Need help configuring customer portal roles",
            "description": "Admin wants a concise walkthrough for portal role setup.",
            "raised_by": "admin@contoso.example",
    ...
    for ticket in tickets:
        existing = await frappe.find_one(
            "HD Ticket",
            filters={"subject": ticket["subject"], "raised_by": ticket["raised_by"]},
            fields=["name", "subject", "raised_by", "status", "priority"],
        )
        seeded_tickets.append(existing or await frappe.create_doc("HD Ticket", clean_payload(ticket)))

    articles = [
        {
            "title": "Reset a customer portal password",
            "content": "<p>Use the user profile menu to trigger a password reset email.</p>",
            "published": 1,
        },
        {
            "title": "Troubleshoot delayed notification emails",
            "content": "<p>Check email queue health, SMTP credentials, and background workers.</p>",
            "published": 1,
        },
    ]
    seeded_articles = []
    for article in articles:
        existing = await frappe.find_one("HD Article", filters={"title": article["title"]}, fields=["name", "title"])
        try:
            seeded_articles.append(existing or await frappe.create_doc("HD Article", article))
        except Exception as exc:
            seeded_articles.append({"title": article["title"], "error": str(exc)})
